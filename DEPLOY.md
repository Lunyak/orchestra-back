...
На сервере поднимаются **PostgreSQL**, **NestJS-бэкенд** и (опционально) **Dozzle** для логов. Electron-приложение у пользователей локальное, оно подключается к API по адресу сервера.
...
API будет доступен по адресу: **http://213.226.126.196:3000**

В Electron-приложении укажите этот URL как базовый для API (например, в настройках или через переменную окружения).
...
При пуше в `master` можно автоматически деплоить на сервер. Настройка:
