...
export function RehearsalsPage({
  ...
  const [weekStart, setWeekStart] = useState(() => isoDate(startOfWeekMonday(new Date())));
  ...
  const createForDay = async () => {
    ...
    setRehearsals((prev) => [...prev, created].sort((a, b) => +new Date(a.startsAt) - +new Date(b.startsAt)));
    setActiveRehearsalId(created.id);

    // по умолчанию: все unknown (пока никто не отметил)
  ...
  const setStatus = async (email: string, status: RehearsalParticipantStatus) => {
    ...
    // сброс кэша плана для репетиции (пересчёт)
    setPlanCache((p) => {
      const next = { ...p };
  ...
  return (
    <div className="rehearsals-page">
      ...
      {!accessToken && <div className="rehearsals-muted">Нужно войти, чтобы видеть репетиции.</div>}
      ...
      <div className="rehearsals-layout">
        <div className="rehearsals-main">
          ...
          <div className="rehearsals-toolbar">
            ...
            <button type="button" onClick={createForDay} disabled={!accessToken || loading}>
              + Создать репетицию
            </button>
          </div>
        ...
        <aside className="rehearsals-side">
          {!activeRehearsal ? (
            ...
            <div className="rehearsals-card">
              ...
              <div className="rehearsals-section">
                <div className="rehearsals-section-title">Что можно репетировать</div>
...
function RehearsalPlanBlock({ accessToken, rehearsalId }: { accessToken: string | null; rehearsalId: string }) {
  const [loading, setLoading] = useState(false);
  ...
  return (
    <div className="rehearsals-plan">
      {bad.length === 0 ? (
        <div className="rehearsals-ok">Собирается (по назначениям ролей).</div>
      ) : (
