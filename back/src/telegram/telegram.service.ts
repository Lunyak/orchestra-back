...
export class TelegramService implements OnModuleInit {
  ...
  constructor(
    ...
    if (!token) {
      console.warn(
        '⚠️ BOT_TOKEN not set or empty, telegram bot will not start',
      );
    ...
    console.log(
      `🤖 Initializing Telegram bot (token: ${token.substring(0, 10)}...)`,
    );
  ...
  private async launchBotAsync() {
    try {
      ...
      const launchOptions = {
        ...
        allowedUpdates: ['message', 'callback_query', 'inline_query'] as const,
      };
      ...
      if (err?.response?.error_code) {
        console.error(
          `   Telegram API error: ${err.response.error_code} - ${err.response.description}`,
        );
      }
      ...
      console.warn(
        '   - Нет доступа к api.telegram.org (проверьте сеть/firewall)',
      );
