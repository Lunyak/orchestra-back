...
import playlistBg from "../../assets/fon_playlist.png";
import "./style.css";
...
export const PlaylistSidebar: React.FC<PlaylistSidebarProps> = ({
  ...
  const resolveTrackSrc = useCallback((file: string) => {
    const url = new URL(`project-audio://${encodeURIComponent(projectName)}/`);
    url.pathname = `/${file}`;
    return url.toString();
  }, [projectName]);
  ...
  const addTracks = async () => {
    try {
      ...
      const newTracks: PlaylistTrack[] = res.tracks.map(
      ...
      const nextTracks = [...playlist, ...newTracks];
  ...
  const addTracksFromPaths = async (filePaths: string[]) => {
    try {
      ...
      await updatePlaylist([...playlist, ...newTracks]);
    } catch (err) {
